...
class Webkul_Customerprofile_Model_Uploadimage {
    var $the_file;
	var $the_temp_file;
    var $upload_dir;
	var $replace;
	...
	function move_upload($tmp_file, $new_file) {
		...
		if ($this->existing_file($new_file)) {
			...
			$fileext=explode(".",$newfile);
			$newfile=  str_replace(" ", "_",$newfile );
			$newfile=  str_replace("-", "_",$newfile );
			if ($this->check_dir($this->upload_dir)) {
				if (move_uploaded_file($tmp_file, $newfile)) {
					$my_upload2 = Mage::getModel('customerprofile/resizeimage');
					$my_upload2->setImage($newfile);
					$my_upload2->resize(300,300,$newfile."2@300@.".$fileext[1]);
					$my_upload2->resize(800,800,$newfile."3@800@.".$fileext[1]);
